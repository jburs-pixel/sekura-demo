...
USAGE:
  ...
  The script will:
   ...
   - Also write a JSON copy to ../deploy/live_ndvi.json for inspection.

  After running, just re-deploy `deploy/` to Netlify Drop. The 6 predios will
  now show real Sentinel-2 NDVI history; the other 144 stay synthetic.
"""
...
def main():
    ...
    ap.add_argument('--html',          default='../deploy/index.html',
                    help='Path to index.html to patch (default: ../deploy/index.html)')
    ap.add_argument('--json',          default='../deploy/live_ndvi.json',
                    help='Where to also dump the raw JSON (default: ../deploy/live_ndvi.json)')
    ...
    print("Next step: re-deploy `deploy/` to Netlify Drop. Done.")
