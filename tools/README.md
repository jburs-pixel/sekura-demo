...
`deploy/index.html` with **real Sentinel-2 observations** fetched from the
...
`LIVE_NDVI` inside `deploy/index.html`. By default that const ships with
...
Wrote ../deploy/live_ndvi.json
Patched ../deploy/index.html  (LIVE_NDVI replaced with 6 predios, 18 lotes)

Next step: re-deploy `deploy/` to Netlify Drop. Done.
```

The script does two things:
1. Writes a JSON copy of the data to `deploy/live_ndvi.json` (handy for inspection)
2. Patches `deploy/index.html` in place — the inline `const LIVE_NDVI = {...}`
   gets swapped for the real data, no other lines change.

## Re-deploy

After running, drag `deploy/` into <https://app.netlify.com/drop>. The 6 live
predios will now show real Sentinel-2 NDVI history; the badge `🛰 Live Sentinel-2 monitoring`
displays everywhere they appear.
